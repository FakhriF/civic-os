...
async function main() {
  ...
  await db
    ...
    .values([
      ...
      {
        name: "Mayor",
        description: "Executive oversight and public reporting view",
      },
  ...
  const adminPassword =
    process.env.SEED_ADMIN_PASSWORD ?? "dev-admin-password-change-me";
  ...
  if (adminRole && popDept && !existingUser) {
    ...
    console.log(
      "⚠️ Default admin user already exists or roles/departments not seeded.",
    );
  }
